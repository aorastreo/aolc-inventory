...
import { stores, pallets, products, productDatabase, adjustments, adjustmentItems, closings, assemblers, assemblyAssignments } from "./schema";
...
async function migrate() {
  ...
  const palletsData: any[] = JSON.parse(adminBackup.pallets);
  const productsData: any[] = JSON.parse(adminBackup.articulos);
  const closingsData: any[] = JSON.parse(adminBackup.cierres);
  const adjustmentsData: any[] = JSON.parse(adminBackup.ajustes);
  const productDbData: any[] = JSON.parse(adminBackup.baseDatos);
  const armadoresData: any[] = JSON.parse(adminBackup.armadores);
  const asignacionesData: any[] = JSON.parse(adminBackup.asignacionesArmado);
  ...
  const palletIdMap = new Map<string, number>(); // old ID -> new DB ID
  ...
  const productIdMap = new Map<number, number>(); // old ID -> new DB ID
  ...
  const adjustmentIdMap = new Map<string, number>(); // old adjustment ID -> new DB ID
  ...
  const assemblerIdMap = new Map<number, number>();
  ...
  for (const a of asignacionesData) {
    ...
    if (!newAssemblerId || !newPalletId) continue;
