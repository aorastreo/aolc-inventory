...
const inventoryRouter = createRouter({
  ...
  // ========== PALLETS ==========
  pallets: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  palletById: publicQuery.input(z.object({ id: z.number() })).query(async ({ input }) => {
    const db = getDb();
    const result = await db.select().from(pallets).where(eq(pallets.id, input.id)).limit(1);
    return result[0] || null;
  }),
  createPallet: publicQuery.input(z.object({ storeId: z.number(), palletId: z.string(), description: z.string(), fecha: z.string().optional(), costo: z.string().optional() })).mutation(async ({ input }) => {
    const db = getDb();
  ...
  updatePallet: publicQuery.input(z.object({ id: z.number(), description: z.string().optional(), costo: z.string().optional() })).mutation(async ({ input }) => {
    const db = getDb();
    const { id, ...data } = input;
    await db.update(pallets).set(data).where(eq(pallets.id, id));
    return { success: true };
  }),
  deletePallet: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    await db.update(pallets).set({ isActive: false }).where(eq(pallets.id, input.id));
    return { success: true };
  }),
  palletsWithStats: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  searchProductsSimilar: publicQuery.input(z.object({ storeId: z.number(), query: z.string() })).query(async ({ input }) => {
    const db = getDb();
  ...
  // ========== PRODUCTS ==========
  products: publicQuery.input(z.object({ storeId: z.number(), palletId: z.number().optional() })).query(async ({ input }) => {
    const db = getDb();
    if (input.palletId) {
  ...
  createProduct: publicQuery.input(z.object({ storeId: z.number(), palletId: z.number(), nombre: z.string(), precio: z.string(), cantidad: z.number().default(1), codigoBarras: z.string().optional(), codigo: z.string().optional(), esNuevo: z.boolean().default(false) })).mutation(async ({ input }) => {
    const db = getDb();
    const result = await db.insert(products).values(input);
    return { id: Number(result[0].insertId) };
  }),
  updateProduct: publicQuery.input(z.object({ id: z.number(), nombre: z.string().optional(), precio: z.string().optional(), cantidad: z.number().optional(), codigoBarras: z.string().optional() })).mutation(async ({ input }) => {
    const db = getDb();
    const { id, ...data } = input;
    await db.update(products).set(data).where(eq(products.id, id));
    return { success: true };
  }),
  deleteProduct: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    await db.update(products).set({ isActive: false }).where(eq(products.id, input.id));
    return { success: true };
  }),
  searchProducts: publicQuery.input(z.object({ storeId: z.number(), query: z.string() })).query(async ({ input }) => {
    const db = getDb();
  ...
  // ========== PRODUCT DATABASE ==========
  productDatabase: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  searchProductDB: publicQuery.input(z.object({ storeId: z.number(), query: z.string() })).query(async ({ input }) => {
    const db = getDb();
  ...
  createProductDB: publicQuery.input(z.object({ storeId: z.number(), nombre: z.string(), precio: z.string().optional(), codigoBarras: z.string().optional(), categoria: z.string().optional() })).mutation(async ({ input }) => {
    const db = getDb();
    const result = await db.insert(productDatabase).values(input);
    return { id: Number(result[0].insertId) };
  }),
  updateProductDB: publicQuery.input(z.object({ id: z.number(), nombre: z.string().optional(), precio: z.string().optional(), codigoBarras: z.string().optional(), categoria: z.string().optional() })).mutation(async ({ input }) => {
    const db = getDb();
    const { id, ...data } = input;
    await db.update(productDatabase).set(data).where(eq(productDatabase.id, id));
    return { success: true };
  }),
  deleteProductDB: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    await db.update(productDatabase).set({ isActive: false }).where(eq(productDatabase.id, input.id));
    return { success: true };
  }),
  adjustmentsWithStats: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  // ========== ADJUSTMENTS ==========
  adjustments: publicQuery.input(z.object({ storeId: z.number(), palletId: z.number().optional() })).query(async ({ input }) => {
    const db = getDb();
    if (input.palletId) {
  ...
  adjustmentById: publicQuery.input(z.object({ id: z.number() })).query(async ({ input }) => {
    const db = getDb();
    const result = await db.select().from(adjustments).where(eq(adjustments.id, input.id)).limit(1);
    return result[0] || null;
  }),
  adjustmentItems: publicQuery.input(z.object({ adjustmentId: z.number() })).query(async ({ input }) => {
    const db = getDb();
    return db.select().from(adjustmentItems).where(eq(adjustmentItems.adjustmentId, input.adjustmentId));
  }),
  createAdjustment: publicQuery.input(z.object({ storeId: z.number(), palletId: z.number(), adjustmentId: z.string(), description: z.string().optional(), fecha: z.string().optional(), fechaHora: z.string().optional() })).mutation(async ({ input }) => {
    const db = getDb();
    const result = await db.insert(adjustments).values({ ...input, estado: "activo" });
    return { id: Number(result[0].insertId) };
  }),
  addAdjustmentItem: publicQuery.input(z.object({ adjustmentId: z.number(), nombre: z.string(), precio: z.string(), cantidad: z.number().default(1), codigoBarras: z.string().optional(), orden: z.number().default(1) })).mutation(async ({ input }) => {
    const db = getDb();
    const result = await db.insert(adjustmentItems).values(input);
    return { id: Number(result[0].insertId) };
  }),
  removeAdjustmentItem: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    await db.delete(adjustmentItems).where(eq(adjustmentItems.id, input.id));
    return { success: true };
  }),
  completeAdjustment: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    await db.update(adjustments).set({ estado: "completado", completedAt: /* @__PURE__ */ new Date() }).where(eq(adjustments.id, input.id));
    return { success: true };
  }),
  cancelAdjustment: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    await db.update(adjustments).set({ estado: "cancelado", isActive: false }).where(eq(adjustments.id, input.id));
    return { success: true };
  }),
  // ========== CLOSINGS ==========
  closings: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
    return db.select().from(closings).where(eq(closings.storeId, input.storeId)).orderBy(desc(closings.fecha));
  }),
  createClosing: publicQuery.input(z.object({ storeId: z.number(), fecha: z.string(), dia: z.string().optional(), efectivo: z.string().default("0"), sinpe: z.string().default("0"), tarjeta: z.string().default("0"), sinFactura: z.string().default("0"), total: z.string().default("0"), inicial: z.string().default("50000"), semana: z.number().optional(), anio: z.number().optional() })).mutation(async ({ input }) => {
    const db = getDb();
    const result = await db.insert(closings).values(input);
    return { id: Number(result[0].insertId) };
  }),
  updateClosing: publicQuery.input(z.object({ id: z.number(), fecha: z.string().optional(), dia: z.string().optional(), efectivo: z.string().optional(), sinpe: z.string().optional(), tarjeta: z.string().optional(), sinFactura: z.string().optional(), total: z.string().optional() })).mutation(async ({ input }) => {
    const db = getDb();
    const { id, ...data } = input;
    await db.update(closings).set(data).where(eq(closings.id, id));
    return { success: true };
  }),
  deleteClosing: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    await db.delete(closings).where(eq(closings.id, input.id));
    return { success: true };
  }),
  closingStats: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  closingTrend: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
    const all = await db.select().from(closings).where(eq(closings.storeId, input.storeId)).orderBy(desc(closings.fecha));
  ...
  closingByPaymentMethod: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  closingWeeklyBreakdown: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
    const all = await db.select().from(closings).where(eq(closings.storeId, input.storeId)).orderBy(desc(closings.fecha));
    const now = /* @__PURE__ */ new Date();
    const dayOfWeek = now.getDay();
  ...
  closingReportByPeriod: publicQuery.input(z.object({ storeId: z.number(), desde: z.string(), hasta: z.string() })).query(async ({ input }) => {
    const db = getDb();
    const all = await db.select().from(closings).where(eq(closings.storeId, input.storeId)).orderBy(desc(closings.fecha));
  ...
  adjustmentItemsForLabels: publicQuery.input(z.object({ storeId: z.number(), adjustmentId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  productsForLabels: publicQuery.input(z.object({ storeId: z.number(), palletId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  markLabelsPrinted: publicQuery.input(z.object({ storeId: z.number(), palletId: z.number(), productIds: z.array(z.number()) })).mutation(async ({ input }) => {
    const db = getDb();
  ...
  unmarkLabelPrinted: publicQuery.input(z.object({ storeId: z.number(), palletId: z.number(), productId: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
  ...
  // ========== ASSEMBLERS ==========
  assemblers: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
    return db.select().from(assemblers).where(and(eq(assemblers.storeId, input.storeId), eq(assemblers.isActive, true)));
  }),
  createAssembler: publicQuery.input(z.object({ storeId: z.number(), nombre: z.string(), telefono: z.string().optional() })).mutation(async ({ input }) => {
    const db = getDb();
    const result = await db.insert(assemblers).values(input);
    return { id: Number(result[0].insertId) };
  }),
  deleteAssembler: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
    await db.update(assemblers).set({ isActive: false }).where(eq(assemblers.id, input.id));
    return { success: true };
  }),
  // ========== DASHBOARD STATS ==========
  dashboardStats: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  // ========== EMPLOYEES ==========
  employees: publicQuery.input(z.object({ storeId: z.number() })).query(async ({ input }) => {
    const db = getDb();
  ...
  createEmployee: publicQuery.input(z.object({ storeId: z.number(), username: z.string(), password: z.string(), name: z.string(), role: z.enum(["employee", "manager", "admin"]).default("employee") })).mutation(async ({ input }) => {
    const db = getDb();
  ...
  deleteEmployee: publicQuery.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db = getDb();
