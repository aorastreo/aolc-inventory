    ...
    try {
        ...
        if (palletsCheck[0].count > 0) {
            ...
            if (adminCheck.length === 0) {
                ...
                await conn.execute(`INSERT INTO employees (storeId, username, password, name, role) VALUES (1, 'admin', ?, 'Administrador', 'admin')`, [hashed]);
                console.log("[DB-INIT] Admin user created!");
            }
            return;
        }
        // Insert Los Chiles store
        await conn.execute(`INSERT INTO stores (id, name, slug, description) VALUES (1, 'American Outlet Los Chiles', 'los-chiles', 'Tienda principal en Los Chiles')`);
...
async function migrateBackupData(conn) {
    try {
        ...
        for (const prod of productsData) {
            ...
            if (!palletDbId)
                continue;
            await conn.execute(`INSERT INTO products (storeId, palletId, codigo, nombre, precio, cantidad, codigoBarras, esNuevo, ordenAgregacion) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?)`, [palletDbId, prod.codigo || null, prod.nombre, String(prod.precio), prod.cantidad || 1, String(prod.codigoBarras || ""), prod.esNuevo ? 1 : 0, prod.ordenAgregacion || null]);
            productCount++;
        }
        ...
        for (const adj of adjustmentsData) {
            ...
            for (const item of adj.articulos || []) {
                await conn.execute(`INSERT INTO adjustmentItems (adjustmentId, nombre, precio, cantidad, codigoBarras, orden) VALUES (?, ?, ?, ?, ?, ?)`, [adjDbId, item.nombre, String(item.precio), item.cantidad || 1, String(item.codigoBarras || ""), item.orden || 1]);
            }
        }
        // Insert assemblers
        const asmMap = {};
        ...
        for (const a of asignacionesData) {
            ...
            if (asmDbId && pltDbId) {
                await conn.execute(`INSERT INTO assemblyAssignments (storeId, assemblerId, palletId, fecha, items, estado) VALUES (1, ?, ?, ?, ?, ?)`, [asmDbId, pltDbId, a.fecha || null, JSON.stringify(a.articulos || []), a.estado || "pendiente"]);
            }
        }
